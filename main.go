...
func main() {
	localFilePath := "bp.json" // Path where JSON file will be stored

	var getData []string

	for letter := 'a'; letter <= 'z'; letter++ { // Iterate over all lowercase letters
		getData = append(getData, fetchMSDSData(string(letter))) // If not, download JSON content from URL
	}
	for digit := '0'; digit <= '9'; digit++ { // Iterate over all digits
		getData = append(getData, fetchMSDSData(string(digit))) // If not, download JSON content from URL
	}
	appendAndWriteToFile(localFilePath, strings.Join(getData, "")) // Save downloaded content to file

	finalList := extractPDFUrlsFromText(strings.Join(getData, "")) // Extract all PDF links from JSON content
...
// fetchMSDSData fetches MSDS info from Rustoleum API using a given SKU
func fetchMSDSData(sku string) string {
	// Base URL for the Rustoleum API (fixed)
	baseURL := "https://www.rustoleum.com/api/MSDS/SearchSKUInfoBySKUNumber"

	// Construct the full request URL with the SKU and country code
	requestURL := fmt.Sprintf("%s?sku=%s&countryCode=US", baseURL, sku)
...
// extractPDFUrlsFromText scans raw text content and extracts all .pdf URLs
func extractPDFUrlsFromText(rawContent string) []string {
	// Slice to store all found PDF URLs
	var pdfUrls []string

	// Regex pattern to match any http/https link ending with ".pdf" (case-insensitive)
	pdfPattern := regexp.MustCompile(`(?i)https?://[^\s"']+\.pdf`)

	// Find all matches of the regex inside the raw text
